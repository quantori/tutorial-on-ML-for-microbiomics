...
def plot_data_type(df: pd.DataFrame) -> plt.Figure:
    """Generate a violin plot of AUROC scores for data types.
...
def plot_tax_level(df: pd.DataFrame) -> plt.Figure:
    """Generate a violin plot of AUROC scores for taxonomic levels.
...
def plot_ml_alg(df: pd.DataFrame) -> plt.Figure:
    """Generate a violin plot of AUROC scores for ML algorithms.
...
def plot_tax_alg(df: pd.DataFrame) -> plt.Figure:
    """Generate a figure with multiple subplots to visualize the effect of
    different experimental parameters on AUROC.
    ...
    _violinplot(
        df=df[df["Data type"] != "metadata_only"],  # metadata_only does not use taxa
        x="Taxonomic level",
        y="AUROC",
        ax=axes[0],
...
def plot_hist_errs_per_sample(
    count_errs: defaultdict, count_samples: defaultdict
) -> plt.Figure:
    ...
    Args:
        count_samples: for each sample, the number of times that the prediction was wrong
            across all nested CV test reps.
    ...
    fig, ax = plt.subplots()
    ax.hist(
        [count_errs[i] / count_samples[i] * 100 for i in count_errs.keys()], bins=10
    )
    ax.set_xlabel("% errors per sample")
...
def plot_bar_errs_per_sample(
    count_errs: defaultdict, count_samples: defaultdict
) -> plt.Figure:
    """Generate a histogram showing the frequency of %errors per sample.

    Args:
        count_samples: for each sample, the number of times that the prediction was wrong across
            all nested CV test reps.
        count_errs: for each sample, the number of predictions made across all nested CV test reps.
    ...
    """
    fig, ax = plt.subplots()
    x = list(range(len(count_errs.keys())))
    y = sorted(
        [count_errs[i] / count_samples[i] * 100 for i in count_errs.keys()],
        reverse=True,
    )
    ax.bar(x, y)
    ax.set_xlabel("Sample")
    ax.set_ylabel("% errors per sample")
    ax.set_xlim(0, 100)
    ax.set_ylim(0, 100)
...
def plot_demographic_errs(
    count_samples: defaultdict,
    count_errs: defaultdict,
    df_metadata: pd.DataFrame,
) -> plt.Figure:
    """Generate a figure with multiple subplots to visualize the effect of
    different demographic factors on model error rate.
    ...
    # Gender
    x_gender, y_gender = get_data(
        "Gender (1:male, 2:female)", count_samples, count_errs, df_metadata
    )
    ...
    # Dwelling condition
    x_dwelling, y_dwelling = get_data(
        "Dwelling condition", count_samples, count_errs, df_metadata
    )
    ...
    # Sample center
    x_center, y_center = get_data(
        "Sample center", count_samples, count_errs, df_metadata
    )
    ...
    # BMI
    _, _ = get_data("BMI", count_samples, count_errs, df_metadata)
    _, _ = get_data("BMI", count_samples, count_errs, df_metadata)
