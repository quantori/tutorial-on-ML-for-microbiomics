...
def baseline1(y_train: pd.Series, X_test: pd.DataFrame) -> list:
    """Train a baseline model that always predicts the majority class (binary)
...
def baseline2(y_train: pd.Series, X_test: pd.DataFrame, random_state: int) -> list:
    """Train a baseline model that predicts binary classes based on their
    ...
    Returns:
        A list of len(y_train) in which all elements are one of two classes, in proportion to
        their frequency within the training data.
    """
...
def baseline345(
    ...
    """Train a baseline random forest model using y_train values that have been
    randomly shuffled.
...
def save_results(
    ...
    save_name: Literal[
        ...
        "baseline_lightGBM",
        "baseline_logistic_regression_L1",
    ],
    ...
    """Save results of k-fold CV experimental condition within the results
    defaultdict.

    Args:
        results: k-fold CV experimental results. For each replicate per data_type, taxonomic_level,
            and alg, you can access train_index, test_index, model_init_seed, AUROC, F1, CM, yhat,
            model.
...
def results_overview_table(results: defaultdict) -> (pd.DataFrame, pd.DataFrame):
    """Generate pandas dataframes that summarize k-fold CV experiments; one for
    easy viewing by readers and one for easy calculation in downstream
    analysis.
    ...
    Returns:
        results_overview: summary of results (e.g., median AUROC, MAD AUROC, etc) for each
            experimental condition (data_type, taxonomic_level, alg).
        results_auroc: data frame where, for each experimental condition (data_type,
            taxonomic_level, alg), there is a column with list of all AUROC scores obtained.
    """
    results_overview = []
    results_auroc = []
    for data_type_key, data_type_value in results.items():
        for tax_level_key, tax_level_value in data_type_value.items():
            for alg_key in tax_level_value.keys():
                out1 = [data_type_key, tax_level_key, alg_key]
                for i in ["AUROC", "F1"]:
                    out1.append(
                        np.median(results[data_type_key][tax_level_key][alg_key][i])
                    )
                    out1.append(
                        median_abs_deviation(
                            results[data_type_key][tax_level_key][alg_key][i]
                        )
                    )
                tn, fp, fn, tp = [], [], [], []
                for i in range(
                    len(results[data_type_key][tax_level_key][alg_key]["CM"])
                ):
                    tn.append(
                        results[data_type_key][tax_level_key][alg_key]["CM"][i][0][0]
                    )
                    fp.append(
                        results[data_type_key][tax_level_key][alg_key]["CM"][i][1][0]
                    )
                    fn.append(
                        results[data_type_key][tax_level_key][alg_key]["CM"][i][0][1]
                    )
                    tp.append(
                        results[data_type_key][tax_level_key][alg_key]["CM"][i][1][1]
                    )
                ...
                out2 = [
                    ...
                    tax_level_key,
                    alg_key,
                    results[data_type_key][tax_level_key][alg_key]["AUROC"],
                ]
                results_auroc.append(out2)

    results_overview = pd.DataFrame(results_overview)
    results_overview.columns = [
        "Data type",
        "Taxonomic level",
        "ML algorithm",
        "Median AUROC",
    ...
    results_overview = results_overview.sort_values(
        by=["Data type", "Taxonomic level", "ML algorithm"], ascending=False
    )
    results_auroc = pd.DataFrame(results_auroc)
    results_auroc.columns = ["Data type", "Taxonomic level", "ML algorithm", "AUROC"]
    results_auroc = results_auroc.sort_values(
        by=["Data type", "Taxonomic level", "ML algorithm"], ascending=False
    )
