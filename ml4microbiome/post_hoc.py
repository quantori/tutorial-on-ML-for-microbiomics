...
def shap_(
    ...
    Returns:
        SHAP values calculated by the SHAP package.
    """
...
def plot_shap(
    shap_dict: dict[int, shap.Explanation],
    X_test_for_shap: list[pd.DataFrame],
    feature_names: list[str],
) -> plt.Figure:
    """Plot SHAP summary bar and beeswarm plots.

    Args:
        shap_dict: keys are indices for reps, values are shap.Explanation outputs (values,
            base_values, data) for each sample in each rep.
        X_test_for_shap: for each rep, the X_test dataset (pandas dataframe) that was used
            to train the model.
...
def errors(yhat: np.ndarray, y_test: pd.DataFrame) -> dict[str, list[int]]:
    """Record sample index of true positives, true negatives, false positives,
    and false negatives.
    ...
    Returns:
        Dictionary where keys = tp/tn/fp/fn, values = counts.
    """
    cm_idx = {"tp": [], "tn": [], "fp": [], "fn": []}
...
def calc_errs_per_sample(
    ...
    """For the k-fold CV experiment, calculate the number of errors per sample
    and number of times each sample was in the test set.

    Args:
        results: k-fold CV experimental results. For each replicate per data type, taxonomic level,
            and ML algorithm, you can access train_index, test_index, model init seed, AUROC, F1,
            CM, yhat, model.
    ...
    Returns:
        errs_per_sample: keys = samples (indexed from 0-n), values = # errors
            per sample in the k-fold CV experiment.
        occurrence: keys = samples (indexed from 0-n), values = # times each
            sample appeared in the test set in the k-fold CV experiment.
    """
    ...
    return errs_per_sample, occurrence


def plot_err_breakdown(
    df_metadata: pd.DataFrame, errs_per_sample: defaultdict, occurrence: defaultdict
) -> plt.Figure:
    """Generate a figure with multiple subplots to visualize the number of
    errors seen in the k-fold CV experiment for selected demographic
    characteristics.

    Args:
        df_metadata: metadata mapping sample to demographic factors.
        errs_per_sample: keys = samples (indexed from 0-n), values = # errors per sample in the
            k-fold CV experiment.
        occurrence: keys = samples (indexed from 0-n), values = # times each sample appeared in the
            test set in the k-fold CV experiment.
    ...
    dfm["n_total"] = [occurrence[i] for i in range(len(dfm))]
    ...
    remap = {
        "married": "Married",
        "single": "Single",
        "single after divorced": "Single after\n divorce",
    }
    ...
    remap = {
        "famliy life": "Family living",
        "group living": "Group living",
        "live alone": "Living alone",
    }
    ...
    remap = {
        ...
        "master degree or above": "Master degree or above",
    ...
    remap = {
        "healthy": "No SCZ",
        "first-episode": "First-episode SCZ",
        "relapse": "Relapse SCZ",
    }
    ...
    x = xticks["Smoking"]
    remap = {
        "never smoke or occasional smoke ": "Non-smoker",
    ...
    remap = {"often": "Medium", "hardly": "Low", "every day": "High"}
    ...
    # Blood pressure
    axes[3, 2].scatter(
        dfm["Systolic pressure(mmHg)"] / dfm["Diastolic pressure(mmHg)"], dfm["errs"]
    )
...
def interaction_heatmap(
    ...
    """Plot a heatmap where each axis is a categorical demographic variable and
    the value/colour per cell shows the number of errors in the k-fold CV for
    samples matching categories within each demographic variable.
    ...
    if var1 == "Education level":
        var1_names = ed_levels
    if var2 == "Education level":
        var2_names = ed_levels

    if var1 == "Gender (1:male, 2:female)":
        var1_names = ["Female", "Male"]
    if var2 == "Gender (1:male, 2:female)":
        var2_names = ["Female", "Male"]
    ...
    for i in range(len(var1_names)):
        for j in range(len(var2_names)):
            ax.text(j, i, out[i][j], ha="center", va="center", color="w")
    ax.grid(None)
...
def plot_distrib_errs(errs_per_sample: defaultdict) -> plt.Figure:
    """Plot the distribution of errors per sample across all replicates of the
    experiment.
...
def dataset_wide_err(errs_per_sample: defaultdict, occurrence: defaultdict):
    """Calculates and prints dataset-wide error rate for a k-fold CV
    ...
    Args:
        ...
        occurrence: keys = samples (indexed from 0-n), values = # times each sample appeared in the
            test set in the k-fold CV experiment.
    """
    err = sum(errs_per_sample.values()) / sum(occurrence.values()) * 100
